    ...
    response.on('end', function (): any {
      ...
      jobs.forEach((job: JobBoardPost) => {
        ...
        sitemapUrls.push({
          name: 'url',
          children: [
            { name: 'loc', text: `${req.protocol}://${req.hostname}${req.originalUrl.replace('/sitemap', '/jobs')}/${job.id}/${job.title}` },
      ...
      jobs.forEach((job: JobBoardPost) => {
        ...
        jobListings.children.push({
          ...
          children: [
            ...
            { name: 'link', text: `${req.protocol}://${req.hostname}${req.originalUrl.replace('/feed', '/jobs')}/${job.id}/${job.title}`},
          ],
        });
      });
